...
def logsumexp(arr):
    arr = np.array(arr)  # Convert input to numpy array for convenience
    max_val = np.max(arr)  # Find the maximum value in the array
    arr = arr - max_val  # Subtract the maximum value from each element in the array
    exps = np.exp(arr)  # Compute the exponential of each element in the array
    sum_of_exps = np.sum(exps)  # Sum all the exponentials
    result = np.log(sum_of_exps)  # Compute the logarithm of the sum
    result = result + max_val  # Add the maximum value (which was subtracted earlier) back to the result
    return result
